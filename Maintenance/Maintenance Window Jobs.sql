...
SELECT  j.NAME ,--[schedule_uid] AS [ScheduleID],
        '''' + STUFF(STUFF(RIGHT('000000'
                                 + CAST([active_start_time] AS VARCHAR(6)), 6),
                           3, 0, ':'), 6, 0, ':') AS ActiveStartTime ,
        '''' + STUFF(STUFF(RIGHT('000000'
                                 + CAST([active_end_time] AS VARCHAR(6)), 6),
                           3, 0, ':'), 6, 0, ':') AS ActiveEndTime
     --[name] AS [ScheduleName]
        ,
        CASE [msdb].[dbo].[sysschedules].[enabled]
          WHEN 1 THEN 'Yes'
          WHEN 0 THEN 'No'
        END AS [IsEnabled] ,
...
FROM    [msdb].[dbo].[sysschedules]
        JOIN dbo.sysjobschedules js ON js.schedule_id = [msdb].[dbo].[sysschedules].schedule_id
        JOIN sysjobs j ON j.job_id = js.job_id
WHERE   
		--(DATEPART(hh,( CONVERT(DATETIME, ( STUFF(STUFF(RIGHT('000000'+ CAST([active_start_time] AS VARCHAR(6)),6), 3, 0, ':'),6, 0, ':') ), 114) ))  >= 6
  --      AND DATEPART(hh,( CONVERT(DATETIME, ( STUFF(STUFF(RIGHT('000000'+ CAST([active_start_time] AS VARCHAR(6)),6), 3, 0, ':'), 6, 0, ':') ), 114) )) < 8)
        
		--OR 
		
		(DATEPART(hh,( CONVERT(DATETIME, ( STUFF(STUFF(RIGHT('000000'+ CAST([active_start_time] AS VARCHAR(6)),6), 3, 0, ':'),6, 0, ':') ), 114) )) >= 21
        AND DATEPART(hh,( CONVERT(DATETIME, ( STUFF(STUFF(RIGHT('000000'+ CAST([active_start_time] AS VARCHAR(6)),6), 3, 0, ':'),6, 0, ':') ), 114) )) < 23)
		AND [freq_type] <> 1
ORDER BY [NAME]