...
FROM (
	SELECT col.TABLE_SCHEMA + 'Tests' TestSchema, 'test Null' + col.TABLE_NAME + col.COLUMN_NAME AS TestName, 'NULL Test' AS TestType, CASE WHEN col.IS_NULLABLE = 'NO' THEN 515 ELSE 0 END AS ExpectedErrorNumber 
	...
	UNION
	SELECT SCHEMA_NAME(so.schema_id) + 'Tests' TestSchema, 'test Duplicate' + so.[Name] + si.[Name] AS TestName, 'Duplicate Test' AS TestType, 2601 AS ExpectedErrorNumber  
	FROM sys.indexes si
	...
	UNION
	SELECT SPECIFIC_SCHEMA + 'Tests' TestSchema, 'test Success' + SPECIFIC_NAME AS TestName, 'Successful Run' AS TestType, 0 AS ExpectedErrorNumber 
	...
	SELECT    [col].[TABLE_SCHEMA] + 'Tests' [TestSchema] ,
			  ...
			  AND [col].[TABLE_SCHEMA] <> 'WebService'
	...
	UNION
	SELECT    [col].[TABLE_SCHEMA] + 'Tests' [TestSchema] ,
                        'test DefaultValue' + [col].[TABLE_NAME] + [col].[COLUMN_NAME] AS [TestName] ,
