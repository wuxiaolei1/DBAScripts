...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions DIStaging'

-- Re-establish multi-user access
ALTER DATABASE [DIStaging] SET MULTI_USER;

Use DIStaging
GO

--Rematch SQLLogins
if exists (select * from sys.server_principals where name = N'CWSRefreshUser')
BEGIN
...
END
if exists (select * from sys.server_principals where name = N'StatementReminderUser')
BEGIN
	exec sp_change_users_login 'Auto_Fix', 'StatementReminderUser'
END
