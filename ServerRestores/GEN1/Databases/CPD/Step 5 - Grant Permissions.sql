...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - CPD'

-- Re-establish multi-user access
ALTER DATABASE [CPD] SET MULTI_USER;

USE CPD
GO

--Rematch the SQL Logins
--exec sp_change_users_login  'Auto_Fix', 'CPD'
...
BEGIN
	...
	exec sp_addrolemember  'DMC', 'CCCSNT\IT Testing Team'
	exec sp_addrolemember  N'db_datareader', N'CCCSNT\IT Testing Team'
END

-- Apply UAT Users --
--Logins added to database if on server
if exists (select * from sys.server_principals where name = N'CCCSNT\CPD UAT')
BEGIN
	IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'CCCSNT\CPD UAT')
		--EXEC sp_revokedbaccess 'CCCSNT\CPD UAT'
		DROP USER [CCCSNT\CPD UAT]
	CREATE USER [CCCSNT\CPD UAT] FOR LOGIN [CCCSNT\CPD UAT]
	exec sp_addrolemember  'DMC', 'CCCSNT\CPD UAT'
END
