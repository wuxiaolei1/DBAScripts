...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - TaskReminders'

-- Re-establish multi-user access
ALTER DATABASE [TaskReminders] SET MULTI_USER;
...
/* users for COP - taken from $/COP/Database/TaskReminder/Trunk/Redgate Setup */
/* create logins */
USE [master]
GO
/**********************************************
...
BEGIN		
	IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = N'CCCSNT\tskremrest')
		CREATE LOGIN [CCCSNT\tskremrest] FROM WINDOWS WITH DEFAULT_DATABASE=[master];
END
ELSE 
BEGIN
...
IF  EXISTS (SELECT * FROM sys.schemas WHERE name = N'TaskReminderRest')
	DROP SCHEMA [TaskReminderRest]
GO

IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'CCCSNT\nonlivetskremrest')
	DROP USER [CCCSNT\nonlivetskremrest];
	
IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'CCCSNT\tskremrest')
	DROP USER [CCCSNT\tskremrest];		
	
IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'TaskReminderRest')
	DROP USER [TaskReminderRest];
...
IF @@SERVERNAME IN ('VMCLUPRO1DBA01','VMCLUPRO2DBA01','VMCLUPRO4DBA01')
BEGIN
	CREATE USER [TaskReminderRest] FOR LOGIN [CCCSNT\tskremrest] WITH DEFAULT_SCHEMA=[dbo];
	
END 	
ELSE
BEGIN
	CREATE USER [TaskReminderRest] FOR LOGIN [CCCSNT\nonlivetskremrest] WITH DEFAULT_SCHEMA=[dbo];
END;
GO 
...
BEGIN
	EXEC sp_addrolemember N'WebServiceRole', N'TaskReminderRest';
END;
