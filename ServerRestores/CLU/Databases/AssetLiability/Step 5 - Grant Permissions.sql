...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - AssetLiability'

-- Re-establish multi-user access
ALTER DATABASE [AssetLiability] SET MULTI_USER;
...
/* users for COP - taken from $/COP/Database/AssetAndLiability/Trunk/Redgate Setup */
/* create logins */
USE [master]
GO
/**********************************************
...
IF  EXISTS (SELECT * FROM sys.schemas WHERE name = N'AssetsRest')
	DROP SCHEMA [AssetsRest]
GO

IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'CCCSNT\nonliveassetsrest')
	DROP USER [CCCSNT\nonliveassetsrest];
	
IF  EXISTS (SELECT * FROM sys.database_principals WHERE name = N'CCCSNT\assetsrest')
	DROP USER [CCCSNT\assetsrest];		
...
BEGIN
	EXEC sp_addrolemember N'WebServiceRole', N'AssetsRest';
END;
