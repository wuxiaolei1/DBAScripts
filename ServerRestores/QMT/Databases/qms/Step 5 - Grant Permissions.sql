...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - QMS'
...
BEGIN
	exec sp_change_users_login  'Auto_Fix', 'qms'
END
if exists (select * from sys.server_principals where name = N'qmatic')
BEGIN
	exec sp_change_users_login  'Auto_Fix', 'qmatic'
END
