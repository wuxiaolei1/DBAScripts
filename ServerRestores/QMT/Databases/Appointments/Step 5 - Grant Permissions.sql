...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - Appointments'
...
BEGIN
	exec sp_change_users_login  'Auto_Fix', 'TCSAppointmentsUser'
END
if exists (select * from sys.server_principals where name = N'TCSBackgroundSvcUser')
BEGIN
	exec sp_change_users_login  'Auto_Fix', 'TCSBackgroundSvcUser'
END
