...
:on error exit
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - ClientRetention'

-- Re-establish multi-user access
ALTER DATABASE [ClientRetention] SET MULTI_USER;

USE ClientRetention
GO

--Rematch the SQL Logins
exec sp_change_users_login  'Auto_Fix', 'ClientRetention'
exec sp_change_users_login  'Auto_Fix', 'ReportingServices'
exec sp_change_users_login  'Auto_Fix', 'PDDUser'
if exists (select * from sys.server_principals where name = N'DCSSQLServerLink')
BEGIN
	exec sp_change_users_login 'auto_fix', 'DCSSQLServerLink'
END
