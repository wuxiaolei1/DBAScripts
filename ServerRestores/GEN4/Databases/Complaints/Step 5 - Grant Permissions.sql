...
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - Complaints'

-- Re-establish multi-user access
ALTER DATABASE [Complaints] SET MULTI_USER;
GO
...
IF @@SERVERNAME IN ('LDSGENPRO1DBA04')
BEGIN
	IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'ComplaintsAPI')
		CREATE USER [ComplaintsAPI] FOR LOGIN [CCCSNT\ComplaintsAPI];
END
ELSE
BEGIN
	/* check if user is linked to login and drop if not */
	IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'ComplaintsAPI')
		CREATE USER [ComplaintsAPI] FOR LOGIN [CCCSNT\NonLiveComplaintsAPI]
	ELSE 
	BEGIN	
		IF NOT EXISTS (	SELECT * FROM sys.database_principals db 
						INNER JOIN sys.server_principals sv 
							ON sv.sid = db.sid
						WHERE db.name = N'ComplaintsAPI')
		BEGIN	
			ALTER USER [ComplaintsAPI] WITH LOGIN = [CCCSNT\NonLiveComplaintsAPI], NAME = [ComplaintsAPI];
		END 
	END 
	;
END
...
IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'ReportingServices')
	CREATE USER [ReportingServices] FOR LOGIN [ReportingServices];
GO

/* Assign Roles */
IF EXISTS (	SELECT * FROM sys.database_principals WHERE name = 'WebServiceRole' AND type = 'R' )
BEGIN
	EXEC sp_addrolemember N'WebServiceRole', N'ComplaintsAPI';
END;
GO

IF EXISTS (	SELECT * FROM sys.database_principals WHERE name = 'ReportingServicesRole' AND type = 'R' )
BEGIN
	EXEC sp_addrolemember N'ReportingServicesRole', N'ReportingServices';
END;
GO
