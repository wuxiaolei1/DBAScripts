...
SET NOCOUNT ON
Print 'Step 5 - Grant Permissions - Reconciliation'

-- Re-establish multi-user access
ALTER DATABASE [Reconciliation] SET MULTI_USER;
GO
...
if exists (select * from sys.server_principals where name = N'CCCSNT\NonLiveRecRestSvc')
BEGIN
	ALTER USER [RecRestSvc] WITH LOGIN = [CCCSNT\NonLiveRecRestSvc], NAME = [RecRestSvc]
END
GO
...
USE [Reconciliation]
GO
ALTER ROLE [db_owner] ADD MEMBER [CCCSNT\IT Analysis]
GO
